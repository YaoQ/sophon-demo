                ...
                if (len(img_list) == batch_size or cn == len(filenames)) and len(img_list):
                    ...
                    for i, filename in enumerate(filename_list):
                        ...
                        for idx in range(det.shape[0]):
                            ...
                            category_id = det[idx][15]
                            landmarks = det[idx][5:15]
                            ...
                            bbox_dict['score'] = float(round(score,4))
                            res_dict['bboxes'].append(bbox_dict)
                            res_dict['landmarks'].append(landmarks.tolist())
    ...
    # test video
        ...
        while not end_flag:
            ...
            if (len(frame_list) == batch_size or end_flag) and len(frame_list):
                det = yolov5(frame_list)
                for i, frame in enumerate(frame_list):
                    cn += 1
                    logging.info("{}, det nums: {}".format(cn, det.shape[0]))

                    res_frame = draw_numpy(frame_list[i], det[:,:4], masks=None, classes_ids=det[:, 15], conf_scores=det[:, 4], landmarks=det[:, 5:15])
                    out.write(res_frame)
                frame_list.clear()
        cap.release()
        out.release()
...
def argsparser():
    ...
    parser.add_argument('--bmodel', type=str, default='../models/BM1684/yolov5s_face_3output_fp32_1b.bmodel', help='path of bmodel')
